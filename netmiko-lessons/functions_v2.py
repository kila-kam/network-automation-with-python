...
#This Module checks for interface statistics
...
#This Module checks for BGP informaion on
def bgp_vb(device):
    try:
        device = ConnectHandler(**device)
        device.enable()
...
def bgp_eurex(device):
    try:
        ...
        #print(output)
        if ( eigrp.count("1.2.3.21")  or eigrp.count("1.2.3.22") ) == 1:
...
def show_security_flow_session(device,prefix):
    ...
    try:
        ...
        for i in prefix:
                ...
                print(" for {} the number of sessions is  {}".format(i,session_count))
    except  Exception as e:
        print(e)
...
def vpn():
  ...
  print("\n\n")
  print('Total number of Active IPSec VPNs: {} ' .format(total))
